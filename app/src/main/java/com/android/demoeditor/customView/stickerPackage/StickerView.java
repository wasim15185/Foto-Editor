...
/**
 * This Sticker View is Modified by <b> Wasim Akram Biswas <b/>
 */
...
public class StickerView extends FrameLayout {
    ...
    private StickerPosition positionListener;
    ...
    /**
     * @{borderpaint} is for ICON COLOR , COLOR_ALPHA
     */
    ...
    private float stickerBorderThickness = 5f;
    ...
    public StickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            /** sticker rect line thickness  & wasim added this */
            /** Wasim added */
            borderPaint.setStrokeWidth(stickerBorderThickness);
...
//        ContextCompat.getDrawable(getContext(), R.drawable.sticker_view_close_icon_18),
                ContextCompat.getDrawable(getContext(), R.drawable.sticker_view_close_icon_12),
                BitmapStickerIcon.LEFT_TOP);
        /**
         * Setting Delete Icon Event
         */
        deleteIcon.setIconEvent(new DeleteIconEvent());
        BitmapStickerIcon zoomIcon = new BitmapStickerIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.sticker_view_resize_12),
                BitmapStickerIcon.RIGHT_BOTOM);
        /**
         * Setting Zoom Icon Event
         */
        zoomIcon.setIconEvent(new ZoomIconEvent());
        BitmapStickerIcon flipIcon = new BitmapStickerIcon(
                ContextCompat.getDrawable(getContext(), R.drawable.sticker_view_flip_icon_12),
                BitmapStickerIcon.RIGHT_TOP);
        /**
         * Setting Flip Icon Event
         */
     ...
     * It is called to draw the child views .
     * Here child view is `Sticker` class
     * inside {@link #dispatchDraw(Canvas)} method array of Stickers is called
     *
     * @param canvas
     */
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        canvas.clipRect(leftX, topY, rightX, bottomY); /** Only This Line Wasim added */
        drawStickers(canvas);
    }
    ...
    /**
     * {@link #onInterceptTouchEvent(MotionEvent)} decides that touch event pass to further (child view/view-group) or return to parent (ViewGroup)
     * {@link #onInterceptTouchEvent(MotionEvent)} is returned 'false' it pass event pass to next (child view/view-group) .
     * If {@link #onInterceptTouchEvent(MotionEvent)}  is returned 'true' it do not pass 'event' to next (child view/view-group) otherwise it pass 'event' to previous view (child view/view-group)
     * <p>
     * when {@link #onInterceptTouchEvent(MotionEvent)} is called after it immediate called {@link #onTouchEvent(MotionEvent)} .
     * .
     * {@link #onTouchEvent(MotionEvent)} works your view not interest any touch event
     *
     * @param ev
     * @return
     */
    ...
    protected void handleCurrentMode(@NonNull MotionEvent event) {
        switch (currentMode) {
            ...
            case ActionMode.DRAG:
                stickerCurrentPos(event);

                /** WASIM added '&& !isStopDrag' this part */
                if (handlingSticker != null && !isStopDrag) {
    ...
    public Bitmap createBitmap() throws OutOfMemoryError {
        ...
        // getting StickerViewRect
        Rect stickerViewRect = getStickerViewRect();

        // getting ImgRect
        Rect imgRect = getImgRect();

        int top = imgRect.top - stickerViewRect.top;
        int left = imgRect.left - stickerViewRect.left;

        int img_height = imgRect.height();
        int img_width = imgRect.width();


        return Bitmap.createBitmap(bitmap, left, top, img_width, img_height);

    }


    @NonNull
    public Bitmap createBitmap(int height, int width) throws OutOfMemoryError {
        handlingSticker = null;
    ...
    @NonNull
    public Bitmap createBitmap(Rect frame) throws OutOfMemoryError {
    ...
    private void stickerCurrentPos(MotionEvent e) {
        if (positionListener != null) positionListener.getPosition(e);
    }

    private interface StickerPosition {
        void getPosition(MotionEvent m);
    }

//    public void setPositionListener(StickerPosition positionListener) {
//        this.positionListener = positionListener;
//    }
//
//    public void setStopDrag(boolean stopDrag) {
//        isStopDrag = stopDrag;
//    }
    ...
    /* this below this section all methods are for cutting bitmap from  */
    ...
    /**
     * {@link #getStickerViewRect()} is for getting area of {@link #StickerView} StickerView using {@link Rect}
     *
     * @return rct - {@link Rect}
     */
    ...
    /**
     * {@link #getImgRect()} is for getting
     * position area of {@link Bitmap} of {@link ImageView } . Which is Child-View of {@link #StickerView}.
     *
     * @return imageRect - {@link Rect}
     */
    private Rect getImgRect() {
        int[] p = getBitmapPositionInsideImageView(mainImage);
    ...
    /* These are printing Rect and RectF*/
    private void printRect(Rect r) {

        Log.d(TAG, "Rect: left :-> " + r.left + " right :-> " + r.right + " top :-> " + r.top + " bottom :-> " + r.bottom);

    }

    /* These are printing RectF*/
    private void printRectF(RectF r) {

        Log.d(TAG, "RectF : left :-> " + r.left + " right :-> " + r.right + " top :-> " + r.top + " bottom :-> " + r.bottom);
