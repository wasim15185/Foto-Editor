...
public class TextSticker extends Sticker {
    ...
    final private String TAG = TextSticker.class.getSimpleName();
    ...
    /**
     * 'wasim' updated this function in below
     *
    ...
    public int getTextColor() {
        if (color == 0)
            Log.w(TAG, " getTextColor: color value is 0 so ' It might be you didn't set color value ' ");
        return this.color;
    }

    @NonNull
    public TextSticker setTypeface(@Nullable Typeface typeface, @NonNull String typeFaceName) {
    ...
    public String getTypeFaceName() {
        if (typeFaceName.length() == 0) {
            Log.w(TAG, "getTypeFaceName: 'No typeface is not added' ");
        }
        return typeFaceName;
    }
